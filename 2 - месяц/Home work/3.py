...
class Boss(GameEntity):
    ...
    def choose_defence(self, heroes):
        ...
        self.defence = random_hero.ability
...
class Magic(Hero):
    ...
    def apply_super_power(self, boss, heroes):
        ...
        print(f'Magic {self.name} увеличил атаку героев на {self.boost_amount}!')
...
class Thor(Hero):
    ...
    def apply_super_power(self, boss, heroes):
        stun_chance = randint(1, 10)
        if stun_chance <= 3:
            print(f'Thor {self.name} оглушил босса!')
            boss.defence = SuperAbility.STUN


class Witcher(Hero):
    def __init__(self, name, health, damage):
        super().__init__(name, health, damage, SuperAbility.RESURRECT)
        self.resurrect_chance = 0.1
    ...
    def apply_super_power(self, boss, heroes):
        ...
        for hero in heroes:
            if hero.health <= 0:
                resurrection_attempt = randint(1, 10)
                if resurrection_attempt <= self.resurrect_chance * 10:
                    print(f'Witcher {self.name} оживил {hero.name}!')
                    hero.health = 1
                    resurrected = True
                    break
...
def start_game():
    ...
    thor = Thor('Thor', 290, 25)
    witcher = Witcher('Geralt', 300, 5)
